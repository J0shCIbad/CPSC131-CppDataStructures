...
*	Date:	03-May-2020	
*	Class: 	CPSC131-05
*	Assignment:		Part 5 - Container Review and Analysis
*/
...
namespace    // unnamed, anonymous namespace
{
    ...
    {  // 3a: Search for an element in a vector
      ...
      measure( "Vector", "Search", [&]( const GroceryItem & item) {dataStructureUnderTest.emplace_back(item);}, [&] (auto&) {
		...
		for( auto it = dataStructureUnderTest.begin(); it!=dataStructureUnderTest.end(); it++){
			if( it->upcCode() == target_upc ){
				return it;
			}
		}
		return dataStructureUnderTest.end();
        /////////////////////// END-TO-DO (5) ////////////////////////////
   ...
   void collect_SinglyLinkedList_measurements()
  {
    {  // 1b:  Insert at the back of a singly linked list
      ...
      measure( "SLL", "Insert at the back", noop, [&]( const GroceryItem & item )  {
		...
		for( auto it=dataStructureUnderTest.begin(); it!=dataStructureUnderTest.end(); it++){
			if( std::next(it, 1) == dataStructureUnderTest.end() ){
				dataStructureUnderTest.insert_after(it, item);
			}
		}
        /////////////////////// END-TO-DO (11) ////////////////////////////
    ...
    {  // 2b:  Remove from the back of a singly linked list
      ...
      measure( "SLL", "Remove from the back", noop, [&]( const GroceryItem & ) {
		...
		if( dataStructureUnderTest.empty() ){
			...
			while( std::next(it, 2) != dataStructureUnderTest.end() ){
				it++;
			}
			dataStructureUnderTest.erase_after(it);
		}
        /////////////////////// END-TO-DO (13) ////////////////////////////
  ...
  *********************************************************************************************************************************/
  void collect_BinarySearchTree_measurements()
  {
	...
	dataStructureUnderTest.insert( std::pair<std::string, GroceryItem>(item.upcCode(), item) );
        /////////////////////// END-TO-DO (16) ////////////////////////////
	...
	return dataStructureUnderTest.find( target_upc );
        /////////////////////// END-TO-DO (18) ////////////////////////////
